<template>
	<view class="container">
	  <canvas data-type="2d" id="canvasid" canvas-id='canvasid' class="canvas" :class="env" :style='{width: `${pxWidth}px`, height: `${pxHeight}px`}'></canvas>
	</view>
</template>
	...
	const main = {
	    ...
	    initCanvas(w, h, debug) {
	        ...
	        return new Promise((resolve) => {
	            that.$set(that, 'pxWidth', that.toPx(w))
	            that.$set(that, 'pxHeight', that.toPx(h))
	            that.$set(that, 'debug', debug)
	            resolve()
	        });
	    }
	}
		...
		methods: Object.assign({
			...
			creates(config){
				...
				query.select('#canvasid')
				  ...
				  .exec((res) => {
					if(!that.ctx || (res && res[0])){
						const canvas = res[0].node
						that.ctx = canvas.getContext('2d')
					}
			
					that.pixelRatio = uni.getSystemInfoSync().pixelRatio
					canvas.width = res[0].width * that.pixelRatio
					canvas.height = res[0].height * that.pixelRatio
					that.ctx.scale(that.pixelRatio, that.pixelRatio)
					// that.ctx.fillRect(0, 0, 100, 100)
					that.cInit(config)
				})
			},
			...
			cInit(config){
				...
				this.initCanvas(config.width, height, config.debug)
					.then(() => {
						...
						let queue = [];
						//保留顺序(层级)提取节点
						Object.keys(config).map(node =>{
							if(['images','blocks','texts','lines'].includes(node)){
								if(node == 'images'){
									queue = queue.concat(that.drawArr)
								}else{
									queue = queue.concat( config[node].map( item => {
										item.type = node.slice(0, -1);
						...
						this.ctx.draw(false, () => {
							setTimeout(() => {
								uni.canvasToTempFilePath({
									canvasId: 'canvasid',
									success: (res) => {
										this.$emit('success', {detail:res.tempFilePath});
									},
									fail: (err) => {
										this.$emit('fail', err);
									},
